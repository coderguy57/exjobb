...
\end{align}
%
Note that neither quantity is equal to zero.
...
\end{aligned}
\end{equation}
The solution is valid only for a finite time interval. Let $t = T_1$ denote the time at which $x_2$ diverges. As $t$ goes to $T_1^-$, $u(x,t)$ will go to
\begin{equation}
\begin{aligned}
	\lim_{t \to T_1^-} u(x, t)
...
\end{equation}
%
It is possible to create a new system from the old one that will conserve the constants of motion as well as keep $u(x, t)_t$ continuous. We define the new system by keeping $x_1$ and $m_1$ as they are and setting $x_2$ to be $-\infty$ and keeping $m_2$ equal to $0$. The difference now is that $x_2$ will not always be greater than $x_1$. With this genealization of the solutions the real axis can be thought of as a circle and when one of the $x_i$'s reaches $\infty$ it comes back at $-\infty$. The solution for the new system is then given by the same equations as before. Thus it seems possible to create a weak solution for $u(x,t)$ that is valid for all time as long as we define $u(x, t)$ at the points where $x_i$ goes to $\infty$ to be the limit of the solution as $x_i$ goes to $\infty$.
...
\end{aligned}
\end{equation}
Doing the same for $c_{13}$ and $c_{23}$ we get a system of ODEs,
\begin{equation}
\begin{aligned}
...
\todo{Här är jag osäker på vad som är av värde att visa. Men la till lite grafer. Vi kan prata om vad som ska vara kvar.}
...
\begin{figure}[H]
	...
	\label{fig:u3N}
\end{figure}
In figure (\ref{fig:u3N}) it can be observed that the function behaves like the absolute function with increasing fluctuations away from a center position. The fluctuations scale linearly with the distance from the center. The behavior of $u(x, t)$ for $x$ far from the center is
\begin{equation}
	\lim_{x \to \infty} u(x, t) = \lim_{x \to \infty} \sum_{k = 1}^{N} m_k |x - x_k| = \sum_{k = 1}^{N} m_k |x| + \sgn(x)x_k
\end{equation}
What is interesting here is that the center seems not to be moving from it's inital position.
\begin{figure}[H]
	\begin{subfigure}{0.44\textwidth}
		\includegraphics[width=\textwidth]{graphs/3NInfinite/uMin.pdf}
        \caption{$\min(u(x,t))$ from 0 to 1}
    \end{subfigure}
	\hfill
	\begin{subfigure}{0.44\textwidth}
		\includegraphics[width=\textwidth]{graphs/3NInfinite/movingAvg.pdf}
        \caption{$\min(u(x,t))$ from 0 to 10}
    \end{subfigure}
    \caption{$m_1 = 1$, $x_1 = 0$, $m_1 = 2$, $x_2 = 3$}
	\label{fig:minU}
\end{figure}
Even though the minimum of the function $u(x, t)$ varies it fluctuates around a constant value as can be seen in figure (\ref{fig:minU}).
...
\end{center}
%
The CH equation also have a canonical Hamiltonian structure with the Hamiltonian
\begin{equation}
	H = \frac{1}{2}\sum_{ij} m_i m_j e^{-|x_i-x_j|}.
\end{equation}
%
The Hunter Saxton equation also have a similar canonical Hamiltonian structure with the Hamiltonian \cite{Beals_2001}
\begin{equation}
...
Each row of the Jacobian matrix is the gradient of one of the constants of motion. If the gradients are linearly independent it means that none of the constants of motion can be expressed as a function of the others. Thus the rank of the Jacobian matrix is the number constants of motion they are all functionally independent.
...
The Lax pair can be used to construct the monodromy matrix \cite{Avendano_2008,Krishnaswami_2021}, which is a matrix that encodes the dynamics of the system. We first need to construct the transition matrix $T$. It is defined as the fundamental solution to the linear system
\begin{equation}
...
Let
\begin{equation}
	2(1 + \lambda m_i m_j (x_j - x_i)) = S_{ij}, \qquad
	\lambda m_i m_j (x_j - x_i) = A_{ij}.
\end{equation}
Thus
\begin{equation}
\begin{aligned}
...
This means that after taking the trace of $DM$ we will get a sum of products of $2(\pm1 + A_{ij})$ factors, where $i < j$.
% \todo{
...
% \end{equation}
% }
Next we will focus on the ways to get a term involving only a specific set of $A_{ij}$ factors.

The terms involving the $A_{ij}$ factors can be visualized as a noncrossing cycle in a complete planar graph where the nodes are the $S_k$ and the edges are the $S_{ij}$ factors. Here is an example of the ways to get the term involving only $A_{13}A_{35}$ times a constant with $N = 9$:
\begin{center}
\begin{tikzpicture}
...
To determine the constant term we get from the other factors, we will first consider if there where no nodes between $S_5$ and $S_1$. Then the only factor would be $S_{51} = (2 + A_{51})$ which would result in a constant term of 2, since we don't want to add more $A_{ij}$ factors.
...
\begin{tikzpicture}
    ...
    \foreach \i in {3,4,5,1}{
        \foreach \j in {3,4,5,1}{
            \ifnum\i<\j
            \draw[gray, very thick, loosely dashed] \dotposition{\i} -- \dotposition{\j};
            \fi
        }
...
\begin{tikzpicture} [scale=0.8, baseline=(current bounding box.north)]
    ...
    \draw[thick]
        ({-360/\n * (1 - 1) + 90}:\radius) to 
        node[midway, xshift=5pt, yshift=5pt, fill=white] {$A_{14}$} % Label at the midpoint
        ({-360/\n * (4 - 1) + 90}:\radius);
    \draw[thick]
        ({-360/\n * (3 - 1) + 90}:\radius) to 
        node[midway, xshift=5pt, yshift=-5pt, fill=white] {$A_{35}$} % Label at the midpoint
        ({-360/\n * (5 - 1) + 90}:\radius);
\end{tikzpicture}
\end{center}
...
\end{aligned}
\end{equation}
This means that the last sum in equation \refeq{eq:C2Sum} can be written as
\begin{equation}
