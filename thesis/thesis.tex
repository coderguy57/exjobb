...
\begin{center}
  \begin{tabular}{l|c|l|l}
    ...
    \hline
	Hunter Saxton
	& $2$ & $\sum m_i $ & $(1/2) \sgn(x) |x|^2$ \\
    \hline
    Derivative Burgers
	& $3$ & $\sum m_i $ & $(1/4) \sgn(x) |x|^2$ \\
  \end{tabular}
\end{center}
%
...
Let
\begin{equation}
	2(1 + \lambda m_i m_j (x_j - x_i)) = S_{ij} \\
...
Now let's prove that $C_2 = C_1^2$. The $C_2$ coefficient will be the sum of all the terms involving two $A_{ij}$ factors. All of them will have two edges in the graph. There are three possible configurations of two edges: they may share two vertices, share one vertex, or share none. This will determine their coefficient. Each edge will contribute a factor 2, and the spaces between two edges also contributes a factor of two.

The different configurations of factors $A_{ij}$ is $A_{ij}^2$, $A_{ij}A_{jk}$, and $A_{ij}A_{kl}$, where $i < j < k < l$.
\begin{center}
\begin{tikzpicture}
	\tikzset{>={Stealth[scale=1.5]}}
...
\end{equation}
The $A_{ik} A_{jl}$ configuration can't be directly from the graph since we can't have crossing paths. But by using the following identity
\begin{equation}
...
Taking the square of $C_1$ we get the sums of two edges where they share two edges, one edge and no edges.
\begin{equation}
	\sum_{i \neq j, k \neq l, \lvert \{i,j\} \cap \{k,l\} \rvert = 2} A_{ij} A_{kl} \\
	+ \sum_{i \neq j, k \neq l, \lvert \{i,j\} \cap \{k,l\} \rvert = 1} 2 A_{ij} A_{kl} \\
	+ \sum_{i \neq j, k \neq l, \lvert \{i,j\} \cap \{k,l\} \rvert = 0} 2 A_{ij} A_{kl}.
\end{equation}

The case were $A_{ij}$ and $A_{kl}$ edges crosses eachother was not possible in the construction of the constants $C_i$. But it can be rewritten as a sum of the two non crossing non connected edges from the vertices $i$, $j$, $k$, and $l$.

For example $A_{13}A_{24}$ is equal to $A_{12}A_{34} + A_{14}A_{23}$.
