				...
				for j in range(N):
					for j2 in range(j+1, N):
						for j3 in range(j2+1, N):
							for j4 in range(j3+1, N):
								m = M.minor_submatrix(i4, j4)
								m = m.minor_submatrix(i3, j3)
								m = m.minor_submatrix(i2, j2)
								H2 = H2 + m.minor(i, j)
...
def sub(expr):
	...
	for i in range(1, N+1):
		for j in range(1, N+1):
			expr = expr.subs(E[(i,j)], (x[j-1]-x[i-1]))
	return expr
